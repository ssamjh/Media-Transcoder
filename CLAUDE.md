...
- `backup.py`: daily snapshots of the state database into `config/backups/`,
  taken through SQLite's backup API (`Db.backup_to`) rather than by copying the
  file, because a copy taken mid-WAL-transaction is the corrupt database the
  whole thing insures against. A snapshot is named for the *day* it was taken
  (`state-2026-01-06.db`), so a second run the same day replaces it instead of
  spending a retention slot - `backup.keep = 7` is therefore seven days however
  often the daemon restarts. Due-ness is read off the newest file's mtime
  (`next_due`), not from memory, for the same reason. `engine._backup_loop` runs
  it on its own thread, never as a step in the scheduler, because a scan can run
